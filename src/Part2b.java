...
public class Part2b {
    ...
    private static void process_patient(Patient p) {
        ...
        //System.out.println("Urgency::::::::::::::: " + p.urgency);
        int level = p.urgency;
        Q[level - 1].enqueue(p);
    }
    ...
    private static void check_emergency() {
        ...
        if (random.nextDouble() < Parameters.p_emergency) {
            ...
            int randomePatient = -1;
            int index;

            for (index = 3; index >= 0; index--) {
                if (!Q[index].isEmpty()) {
                    randomePatient = random.nextInt(Q[index].size() - 1);
                    break;
                }
            }
            if (randomePatient != -1) {
                assign_doctor(Q[index].get(randomePatient));
            }
        }
    }
    ...
    private static void check_promotion() {
        ...
        for (int i = 0; i < Q[2].size(); i++) {
            if (Q[2].get(i).wait_time(time) >= 3*(Parameters.l12_escalation_delay)) {
                Q[3].push(Q[2].dequeue(i));
            }
        }*/
    ...
    private static void check_walkout() {
        ...
        if (random.nextDouble() < Parameters.p_walkout) {
            ...
            int randomPatient = random.nextInt(Q[0].size() - 1);
            Q[0].dequeue(randomPatient);
        }
    }
